...
| `tools/playtest.mjs` | — | 이동·사격 스모크 + **창호지 구멍 모델 검사**(구멍 수 = 피격 수, 구멍 UV·무기별 반지름, 찢어짐 임계 — PATCH-014-D) + **탄흔 데칼 부재 클립 검사**(`decal_within_member`: 창살 중앙·가장자리 명중 뒤 A 데칼보임/B 데칼숨김/C 데칼+창살숨김 세 장으로 "데칼이 칠한 픽셀"이 창살 실루엣 1 px 팽창 밖에 있으면 실패 — R4). 음성 `--inject-no-holes`(케이스 26) · `--inject-decal-noclip`(클립 해제 → 종이 위 번짐, 케이스 28) | ✓ |
