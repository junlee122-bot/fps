...
export const GRADE_DEFAULT = Object.freeze({
  ...
  // R1 A-3(2): 상한 적용 위치를 LUT → 출력 셰이더(output.js)로 이동. 32³ LUT는 암부에서 노드 색상(hue)이 픽셀과 무관해 상한이
  // 작동하지 않았다(사전점검 위반 픽셀 전부 v≈.04~.07). gradeColor(오프라인 참조)는 그대로 캡을 포함하고, 파이프라인은
  // bandCap=null로 LUT를 빌드한 뒤 같은 파라미터를 셰이더 유니폼으로 건다(8비트 양자화 여유 포함).
