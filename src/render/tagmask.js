 ...
 *   - 태그하지 않음: FX_PARTICLE, 뷰모델, 하늘 돔, HANJI(투과는 셰이더 가산, emissive 아님)
 *     [R4 작업 2 정정] 종전 근거는 "입자는 무채색 물질"이었는데 더는 참이 아니다 — 입자는 룩 표
 *     (src/materials/fx-look.js)의 색을 싣고 불똥 2종은 자발광 가중이 있다. 그래도 태그하지 않는
 *     이유는 다른 데 있다: 입자 15종은 InstancedMesh **한 개**를 공유하므로 재질 이름 규칙으로는
 *     인스턴스별 태그가 불가능하다. 대신 입자 색은 자발광 면제 없이 팔레트 §4 표를 그대로 지키며,
 *     그 준수는 tools/fxaudit.mjs 가 색 정의 시점에 게이트한다(paletteInBand).
 *
